...
- [views and design functions](#views-and-design-functions)
	- [db.search(designname, viewname, query, [params], [callback])](#dbsearchdesignname-viewname-query-params-callback)
...
### db.search(designname, viewname, query, params, [callback])
...
``` js
alice.search('characters', 'crazy_ones', 'rab*', function(err, body) {
  if (!err) {
    body.rows.forEach(function(doc) {
